...
def erode_dilate(src: MatLike) -> MatLike:
...
def threshold_red(src: MatLike) -> MatLike:
    ...
    hue_range = 15
    low_range = cv2.inRange(src, (0, 0, 150), (hue_range, 255, 255))
    high_range = cv2.inRange(src, (179 - hue_range, 0, 150), (179, 255, 255))
...
def filter_contours(contours: Sequence[MatLike], side: str, height: int, width: int) -> Sequence[MatLike]:
    ...
    if side == LEFT_SIDE:
        contours = list(filter(lambda cnt: get_centroid(cnt)[0] < (width / 2), contours))
...
def pipeline(src: MatLike) -> str:
    ...
    green_threshold_img = erode_dilate(green_threshold_img)
    ...
    red_threshold_img = erode_dilate(red_threshold_img)
